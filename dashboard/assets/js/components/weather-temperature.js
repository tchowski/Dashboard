parasails.registerComponent('weather-temperature', {
    //  ╔═╗╦═╗╔═╗╔═╗╔═╗
    //  ╠═╝╠╦╝║ ║╠═╝╚═╗
    //  ╩  ╩╚═╚═╝╩  ╚═╝
    props: [
    ...
    //  ╠═╣ ║ ║║║║
    //  ╩ ╩ ╩ ╩ ╩╩═╝
    template: `<div class="col-md-6 weather-temperature">
    ...
    methods: {
        ...
        getWeather: async function () {
            await fetch("https://api.openweathermap.org/data/2.5/weather?q=" + this.idWeather + "&APPID=a0ca740beab72b520c6a204b338a938d&units=metric").then(response => response.json()).then(data => this.temperature = data.main.temp).catch(error => console.error(error));
        },
        deleteWidget: function (index) {
            $("div").remove(".weather-temperature");
        },
    }
