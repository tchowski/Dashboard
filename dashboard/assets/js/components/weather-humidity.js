parasails.registerComponent('weather-humidity', {
    //  ╔═╗╦═╗╔═╗╔═╗╔═╗
    //  ╠═╝╠╦╝║ ║╠═╝╚═╗
    //  ╩  ╩╚═╚═╝╩  ╚═╝
    props: [
    ...
    //  ╠═╣ ║ ║║║║
    //  ╩ ╩ ╩ ╩ ╩╩═╝
    template: `<div class="col-md-6 weather-humidity">
    ...
    methods: {
        ...
        getWeather: async function () {
            await fetch("https://api.openweathermap.org/data/2.5/weather?q=" + this.idWeather + "&APPID=a0ca740beab72b520c6a204b338a938d&units=metric").then(response => response.json()).then(data => this.humidity = data.main.humidity).catch(error => console.error(error));
        },
        deleteWidget: function (index) {
            $("div").remove(".weather-humidity");
        },
    }
